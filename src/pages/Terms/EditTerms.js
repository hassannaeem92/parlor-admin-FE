...
import {
  ...
  getspecificTerms,
  updateTerms,
} from "../../store/AsyncMethods/TermsMethod";
import { RESET_SPECIFIC_TERMS } from "../../store/Types/TermsTypes";
...
export default function EditTerms() {
  ...
  const { specificTERMS } = useSelector((state) => state.TermsReducer);

  useEffect(() => {
    if (specificTERMS) {
      formik.setFieldValue("title", specificTERMS?.title);
      formik.setFieldValue("description", specificTERMS?.description);
      formik.setFieldValue("status", specificTERMS?.status ? true : false);
    }
  }, [specificTERMS]);
  const formik = useFormik({
    ...
    onSubmit: async (data) => {
      dispatch(updateTerms(data, id)).then((success) => {
        if (success) {
          formik.resetForm();
          dispatch(getTerms());
          dispatch({ type: RESET_SPECIFIC_TERMS });
          navigate("/terms");
        }
      });
    },
