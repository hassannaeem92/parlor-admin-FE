...
import {
  ...
  getContacts,
} from "../../store/AsyncMethods/ContactMethod";
...
export default function ContactsTable() {
  ...
  const accept = () => {
    dispatch(deleteContacts(selectedItem?.id)).then((success) => {
      if (success) {
  ...
  const acceptMultiple = () => {
    if (selectedProducts) {
      ...
      if (selectedProducts?.length !== 0) {
        dispatch(deleteMultipleContacts(idsArray)).then((success) => {
          if (success) {
    ...
    <div>
      <div className="card shadow-sm">
        <DataTable
          ...
          onRowClick={(e) => navigate("/edit-contact/" + e.data.id)}
        ...
        >
          ...
          <Column field="number_of_persons" sortable header="Number of Persons" />
          <Column field="appointment_date" sortable header="Appointment Date" />
          <Column field="appointment_time" sortable header="Appointment Time" />
          {/* <Column
            field="status"
            sortable
            header="Status"
