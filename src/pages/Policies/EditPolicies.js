...
import { RESET_SPECIFIC_POLICIES } from "../../store/Types/PoliciesTypes";
...
export default function EditPolicies() {
  ...
  const { specificPOLICIES } = useSelector((state) => state.PoliciesReducer);

  useEffect(() => {
    if (specificPOLICIES) {
      formik.setFieldValue("title", specificPOLICIES?.title);
      formik.setFieldValue("description", specificPOLICIES?.description);
      formik.setFieldValue("status", specificPOLICIES?.status ? true : false);
    }
  }, [specificPOLICIES]);
    ...
    onSubmit: async (data) => {
      dispatch(updatePolicies(data, id)).then((success) => {
        if (success) {
          ...
          dispatch({ type: RESET_SPECIFIC_POLICIES });
          navigate("/policies");
        }
      });
    },
  ...
  useEffect(() => {
    dispatch(getspecificPolicies(id));
  }, []);
