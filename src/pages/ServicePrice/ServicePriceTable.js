...
export default function ServicePriceTable() {
  ...
  const { servicePrices } = useSelector((state) => state.ServicePriceReducer);
  ...
  const accept = () => {
    dispatch(deleteServicePrice(selectedItem?.id)).then((success) => {
      if (success) {
  ...
  const acceptMultiple = () => {
    if (selectedProducts) {
      ...
      if (selectedProducts?.length !== 0) {
        dispatch(deleteMultipleServicePrices(idsArray)).then((success) => {
          if (success) {
  ...
  let items = [
    {
      ...
      icon: "pi pi-pencil",
      command: () => {
        navigate(`/edit-service-price/${selectedItem?.id}`);
      },
    },
    {
      label: "Delete",
    ...
    <div>
      <div className="card shadow-sm">
        <DataTable
          ...
          onRowClick={(e) => navigate("/edit-service-price/" + e.data.id)}
        ...
        >
          ...
          ></Column>
          {/* <Column field="name" sortable header="Service Name" /> */}
          {/* <Column field="description" sortable header="Description" /> */}
          <Column field="service_name" sortable header="Service Name" />
          <Column field="sub_service_name" sortable header="Sub Service Name" />
